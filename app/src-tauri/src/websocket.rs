...
impl WebSocketClient {
    ...
    pub async fn connect(&self) -> Result<(), String> {
        ...
        let _ = self.app.emit("websocket_status", &WebSocketStatus::Connecting);
        ...
        match connect_async(&config.url).await {
            Ok((ws_stream, _)) => {
                ...
                // Emit status change event
                let _ = self.app.emit("websocket_status", &WebSocketStatus::Connected);
                
                // Start message handling in background task
                let handle = self.start_message_handling(ws_stream).await;
                
                // Store the connection handle
                {
                    let mut connection_handle = self.connection_handle.lock().unwrap();
            ...
            Err(e) => {
                ...
                // Emit status change event
                let _ = self.app.emit("websocket_status", &WebSocketStatus::Error(error_msg.clone()));
    ...
    async fn start_message_handling(&self, ws_stream: tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            let status_for_read = status_clone.clone();
            ...
            let read_task = tokio::spawn(async move {
                while let Some(msg) = read.next().await {
                    match msg {
                        ...
                        Ok(Message::Close(_)) => {
                            {
                                let mut status = status_for_read.lock().unwrap();
                ...
                {
                    let mut sender = tx_for_read.lock().unwrap();
                    *sender = None;
                }
            });
    ...
    pub async fn disconnect(&self) {
        ...
        // Emit status change event
        let _ = self.app.emit("websocket_status", &WebSocketStatus::Disconnected);
